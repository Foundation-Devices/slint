...
pub(super) fn draw_texture_line(
    ...
    if !rotation.is_transpose() {
        ...
        #[allow(unsafe_code)]
        if alpha == 0xff && delta == Fixed::from_integer(1) {
            let pos = pos.truncate() as usize;
            if format == PixelFormat::Rgb && colorize.alpha() == 0 {
                let (unaligned_part, data, _tail) = unsafe { data.align_to() };
                if unaligned_part.len() == 0 {
            ...
            if format == PixelFormat::RgbaPremultiplied && colorize.alpha() == 0 {
                let (unaligned_part, data, _tail) = unsafe { data.align_to() };
                if unaligned_part.len() == 0 {
            ...
            if format == PixelFormat::AlphaMap && colorize.alpha() == 0xff {
