...
impl RoundedClipRegion {
    ...
    pub fn clip_range_for_line(&self, line_y: i16) -> Option<(i16, i16)> {
        ...
        if left_x < right_x { Some((left_x, right_x)) } else { None }
    }
}
...
///
/// Note: Some commands (like `RoundedRectangle` and the gradients) have their own
...
/// A rendered alpha coverage map, one byte per pixel.
#[derive(Clone)]
