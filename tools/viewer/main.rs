...
fn main() -> Result<()> {
    ...
    if args.auto_reload {
        ...
        #[cfg(feature = "custom-translations")]
        let result = if args.i18n_dir.is_some() && args.locale.is_some() {
            // Inject a TR re-export so custom translations can reach the global.
            let source = std::fs::read_to_string(args.path()).unwrap_or_else(|e| {
                eprintln!("Failed to read main file {}: {e}", args.path().display());
                std::process::exit(-1);
            });
            let source = if source.contains("import { TR }") && !source.contains("export { TR }") {
                // TODO: localize
                format!(
            ...
            poll_ready(compiler.build_from_source(source, args.path().to_path_buf()))
        } else {
            poll_ready(compiler.build_from_path(args.path()))
        };
...
mod custom_translations {
    ...
    pub fn setup_custom_translations(
        ...
        // Try to setup lookup callback. If the TR global doesn't exist the setup fails and we
        // report it rather than silently rendering untranslated text.
